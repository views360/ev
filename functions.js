                   ...
                   <table>
                       <thead>
                           <tr>
                               ...
                               <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)">💡<span class="tooltip-box">This is the provider's subscription fee, which offers access to their discounted charge rate for ONE MONTH. You must follow their instructions to access their discounted rates, which is usually done by paying through their app so they can identify your subscription.</span></span></span>Sub. Fee</th>
                               <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)">💡<span class="tooltip-box">This is the provider's discounted charge rate (per kWh) that is available after subscribing for one month. Note that some providers have variable charge rates depending on location and time of day. The rate listed here may be an average. Click the provider's link to confirm pricing. You must follow the provider's instructions to access discounted rates, which is usually done by paying through their app so they can identify your subscription.</span></span></span>Disc. Rate</th>
                               <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)">💡<span class="tooltip-box">Read carefully. This is the number of miles you must drive on the provider's discounted charge rate to pay off the subscription fee. This number of miles begins at the point when you first charge with the provider after your pre-charge decreases to your recharge threshhold of ${inputs.rechargeAt}%. The number of journeys has no impact.</span></span></span>Break-Even Miles</th>
                           </tr>
                       </thead>
                       <tbody>`;
...
// FIXED: Recalculate pre-charge costs at the PROVIDER's discounted rate
// (not the user's specified startChargeRate and j.rate which are PAYG rates)
let providerPreChargeCost = 0;
const mainProviderTopUpKwh = Math.max(0, ((inputs.soc - inputs.rechargeAt) / 100) * inputs.batteryKwh);
providerPreChargeCost += mainProviderTopUpKwh * (rate / 100);

if (inputs.additionalJourneys.length > 0) {
inputs.additionalJourneys.forEach((j) => {
const extraTopUpKwh = Math.max(0, ((j.soc - inputs.rechargeAt) / 100) * inputs.batteryKwh);
providerPreChargeCost += extraTopUpKwh * (rate / 100);
});
}

const totalJourneyCost = subCost + providerPreChargeCost + totalPublicChargingCost;
...
let html = `<div class="mobile-only-text" style="font-size: 0.8em; text-align: center; color: var(--neon-green)">Slide table left to view hidden columns.</div><div class="results-scroll"><table><thead><tr>
       <th>Provider (click hyperlink to view subscription info)</th>
        ...
        <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)" style="font-size: 0.8rem;">💡<span class="tooltip-box">This is the provider's subscription fee, which offers access to their discounted charge rates for ONE MONTH.</span></span></span>Sub. Fee</th>
       ...
       <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)" style="font-size: 0.8rem;">💡<span class="tooltip-box">This is the total cost difference between using a subscription vs using PAYG. Green means the subscription will be cheaper; red means it will be more expensive.</span></span></span>vs. PAYG</th>
       <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)" style="font-size: 0.8rem;">💡<span class="tooltip-box">Read carefully. This is the number of miles you must drive on the provider's discounted charge rate to pay off the subscription fee. This number of miles begins when you first charge with the provider after your pre-charge decreases to your recharge threshhold of ${inputs.rechargeAt}%. The number of journeys has no impact.</span></span></span>Break-Even Miles</th>
       <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)" style="font-size: 0.8rem;">💡<span class="tooltip-box">Read carefully. This is the break-even miles PLUS the number of miles you drive from your initial pre-charge until you reach your recharge threshhold of ${inputs.rechargeAt}% and begin charging with this provider The number of journeys has no impact.</span></span></span>Break Even + Battery</th>
       </tr></thead><tbody>`;
providers.forEach(p => {
