...
function buildStopsRowsForJourney(journeyMiles, startSoc, rechargeAt, efficiency, batteryKwh) {
    ...
    // Range available from the initial precharge (from startSoc down to rechargeAt)
    ...
    // --- CASE 1: Journey is fully covered by the precharged battery (no public charging) ---
...
function calculate() {
    ...
    if (inputs.additionalJourneys.length > 0) {
        ...
        preChargeHtml = `<p style="margin: 0px;"><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)" style="font-size: 0.8rem;">💡<span class="tooltip-box">This is the cost of precharging your battery at your start/departure location before your journey.</span></span></span>Pre-journey battery charge (${inputs.rechargeAt}%→${inputs.soc}%, ${mainTopUpKwh.toFixed(1)} kWh x ${inputs.startChargeRate}p): 
    ...
    let html = `<div class="mobile-only-text" style="font-size: 0.8em; text-align: center; color: var(--neon-green)">Slide table left to view hidden columns.</div><div class="results-scroll"><table><thead><tr>
        ...
        <th><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)" style="font-size: 0.8rem;">💡<span class="tooltip-box">This is the break-even miles PLUS the initial number of miles your vehicle can drive based on its precharged state. The number of journeys has no impact on this value.</span></span></span>Break Even + Battery</th>
        </tr></thead><tbody>`;
    providers.forEach(p => {
...
function addJourneyField() {
    ...
    journeyDiv.innerHTML = `
        ...
        <div class="input-row">
            ...
            <div class="input-group">
                <label><span class="tooltip-container"><span class="info-icon" onclick="toggleTooltip(this)">💡<span class="tooltip-box">While it is to be expected that you will depart from your usual place (e.g., home) and charge at your usual rate, you may have other plans — so this allows the results to take that into account.</span></span></span>Battery Precharge Rate (p/kWh)</label>
