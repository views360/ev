    ...
    <div class="guide-section" id="payg-summary">
        ...
        <span class="tooltip-container">
            <span class="info-icon" style="font-size:0.8rem" onclick="toggleTooltip(this)">
                💡<span class="tooltip-box">This is the cost to charge your battery from your recharge threshold (${rechargeThreshold}%) up to your starting state of charge (${targetSoc}%).</span>
            </span>
        </span>
        Pre-journey battery charge cost (${rechargeThreshold}% - ${targetSoc}% / ${startChargeKwh.toFixed(1)} kWh): <strong>£${startChargeCost.toFixed(2)}</strong>
    </div>`;
...
const targetSoc = inputs.soc;
const chargeDiffPercent = Math.max(0, targetSoc - rechargeThreshold);
const startChargeKwh = (inputs.soc / 100) * inputs.batteryKwh;
const startChargeCost = startChargeKwh * (inputs.startChargeRate / 100);
const chargeToPercent = 80; // Charge to 80% at each public stop (except final)
