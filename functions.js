...
function calculate() {
    ...
    document.getElementById("preChargeLine").innerHTML = `<div class="guide-section" id="payg-summary"><h3><i>Theoretical</i> PAYG Summary</h3><p style="font-size:0.85rem; margin-top:12px; opacity:0.8;">This is <i>theoretical</i> because it ignores the 80/20 rule, and the statistics are based on draining your pre-charged battery to 0% before recharging, which you should never do.</p>Pre-journey starting charge: <strong>${startChargeKwh.toFixed(1)} kWh</strong> (£${startChargeCost.toFixed(2)})</div>`;
