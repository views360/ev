...
function calculate() {
    ...
    if (providers.length > 0) {
        ...
        if (bestProvider.savings > 0) {
            ...
            conclusionHTML += `<h3>PAYG vs SUBSCRIPTION CONCLUSION</h3><p class="main-result">A <strong>${inputs.adhoc}p PAYG rate</strong> is cheaper than a subscription at the selected minimum charging rate of <strong>${minSpeedLabel}</strong> and the other information entered. The total journey cost based on PAYG will be <strong>£${totalAdhocCost.toFixed(2)}</strong>.</p>`;
        }
        conclusionHTML += `</div>`;
