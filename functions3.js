...
const addJourneyBtn = document.querySelector('button[onclick="addAdditionalJourney()"]');
    ...
    if (!isTripMode) {
        ...
        const efficiencyBE = parseFloat(document.getElementById("efficiencyBE").value);
        const adhocRateBE = parseFloat(document.getElementById("adhocBE").value) || 0;
        const minSpeedSelection = parseFloat(document.getElementById("minSpeedBE").value) || 0;

        if (isNaN(efficiencyBE) || efficiencyBE <= 0 || isNaN(adhocRateBE) || adhocRateBE <= 0) {
        ...
        PRESETS.forEach(p => {
            ...
            speedKeys.forEach(speed => {
                ...
                if (rate < adhocRateBE) {
                    const savingPerKwh = (adhocRateBE - rate) / 100;
                    const kwhNeeded = sub / savingPerKwh;
                    breakEvenMiles = Math.round(kwhNeeded * efficiencyBE);
...
conclusionsBox.style.display = "block";
    ...
    if (calcLines) calcLines.style.display = "block";
    if (chartWrapper) chartWrapper.style.display = "block";

    // 4. PRE-JOURNEY & HOME CHARGING
    ...
    const startChargeLine = document.getElementById("startChargeLine");
    const publicKwhLine = document.getElementById("publicKwhLine");
    const adhocCostLine = document.getElementById("adhocCostLine");

    if (startChargeLine) startChargeLine.innerHTML = preJourneyBreakdownHtml;
    if (publicKwhLine) publicKwhLine.innerHTML = inputs.additionalJourneys.length > 0 ? breakoutHtml : "";
...
const providers = [];
    ...
    providerBoxes.forEach(box => {
        ...
        // Calculate Journey 1 for this provider
        ...
        // Calculate additional journeys for this provider
        ...
        const pData = PRESETS.find(p => p.name === (presetEl ? presetEl.value : ""));
...
const maxChargingSpeed = parseFloat(document.getElementById("maxChargingSpeed").value) || 0;
    ...
    // Calculate total stops across all journeys for the average
    let totalStopsCount = j1Sim.stopCount;
    inputs.additionalJourneys.forEach(j => {
        totalStopsCount += runJourneySimulation({
            miles: j.miles,
            batteryKwh: inputs.batteryKwh,
            efficiency: inputs.efficiency,
            rechargeAt: inputs.rechargeAt,
    ...
    chargeSpeeds.forEach(speed => {
        ...
        const avgMins = totalStopsCount > 0 ? Math.round(totalMins / totalStopsCount) : 0;
        ...
        speedTableHtml += `
            <tr style="${isUserMax ? 'background: rgba(34, 197, 94, 0.1); border: 1px solid var(--neon-green);' : 'border-bottom: 1px solid var(--border);'}">
                ...
                <td style="padding: 10px;">${totalStopsCount > 0 ? avgMins + ' mins' : 'N/A'}</td>
            </tr>`;
    });
